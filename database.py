...
# 클라이언트는 첫 사용 시점에 생성 (lazy init) — 모듈 임포트 시 크래시 방지
...
def init_db():
    ...
    try:
        _get_supabase().table("wines").select("id").limit(1).execute()
...
def save_wine(wine_data: dict) -> int:
    ...
    res = _get_supabase().table("wines").insert(row).execute()
...
def get_all_wines(user_name=None, sort_by="date",
    ...
    """조건에 맞는 와인 목록 반환"""
    query = _get_supabase().table("wines").select("*")
...
def get_high_rated_wines(user_name=None, min_rating=4) -> list:
    ...
    query = _get_supabase().table("wines").select("*").gte("rating", min_rating)
    if user_name:
        query = query.eq("user_name", user_name)
...
def delete_wine(wine_id: int):
    """와인 삭제"""
    _get_supabase().table("wines").delete().eq("id", wine_id).execute()


def update_wine_rating(wine_id: int, rating: int, memo: str):
    """평점 및 메모 업데이트"""
    _get_supabase().table("wines").update({"rating": rating, "memo": memo}).eq("id", wine_id).execute()
...
def get_unique_varieties(user_name=None) -> list:
    """고유 포도 품종 목록 반환"""
    query = _get_supabase().table("wines").select("grape_variety")
    if user_name:
        query = query.eq("user_name", user_name)
    res = query.execute()
    varieties = sorted({
...
def get_unique_regions(user_name=None) -> list:
    """고유 지역 목록 반환"""
    query = _get_supabase().table("wines").select("region")
    if user_name:
        query = query.eq("user_name", user_name)
    res = query.execute()
    regions = sorted({
